...
export default function LootCard({
  ...
  useEffect(() => {
    ...
    if (size) customStyles += `height: ${size.height}px; width: ${size.width}px;`;
    if (shadowOptions && shadowOptions.default) customStyles += `box-shadow: -5px -5px 5px -5px ${shadowOptions.default.color1}, 5px 5px 5px -5px ${shadowOptions.default.color2}, -7px -7px 10px -5px transparent, 7px 7px 10px -5px transparent, 0 0 5px 0px rgba(255, 255, 255, 0), 0 55px 35px -20px rgba(0, 0, 0, 0.5);`
    ...
    $cards
      .on("mousemove touchmove", function (e) {
        ...
        const tf = `
          transform: rotateX(${ty}deg) rotateY(${tx}deg) rotateZ(${landscape ? "90deg" : "0"}); 
          ${customStyles}
        `;

        // Need to use a <style> tag for pseudo elements
        let style = "";
        if (rareCards.some((c) => rarityPreset.includes(c)) || holographicOptions?.glow) {
          style = `
      ...
      .on("mouseout touchend touchcancel", function () {
        ...
        if (size) $card.css({ height: `${size.height}px`, width: `${size.width}px`});
        if (animationOptions?.animatedOnMouseOut)
          x = setTimeout(() => $card.addClass("animated"), animationOptions.delayOnMouseOut || 2500);
    ...
    setIsLoaded(true);
  }, [rareCards, rarityPreset, img, isFirstLoad, shineOptions, holographicOptions, shadowOptions, size, animationOptions]);
    ...
    <section className="cards" id="opening">
      <div
        className={`
          ...
          ${rareCards.some((c) => rarityPreset.includes(c) || holographicOptions) ? "holo" : ""} 
